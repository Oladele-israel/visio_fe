      ...
      <div className="relative z-10 w-full max-w-lg bg-card border border-border rounded-xl shadow-2xl">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="relative">
            ...
            <button
              onClick={copy}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 rounded-md text-muted-foreground hover:text-blue-400 hover:bg-blue-500/10 transition-all"
              title="Copy token"
            >
              {copied
          ...
          <Button
            onClick={copy}
            className="w-full bg-blue-600 hover:bg-blue-500 text-white h-10 gap-2"
          >
      ...
      <div className="relative z-10 w-full max-w-md bg-card border border-border rounded-xl shadow-2xl p-6 space-y-5">
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <Button
            onClick={onConfirm}
            disabled={isRevoking}
            className="bg-red-600 hover:bg-red-500 text-white h-9 px-5 gap-2"
          >
            {isRevoking
  ...
  return (
    ...
    >
      <div className="space-y-5">
        {alert && (
          <Alert type={alert.type} message={alert.msg} onDismiss={() => setAlert(null)} />
        )}

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Display Name" icon={<User className="w-3.5 h-3.5" />} error={errors.name}>
            <Input
              value={name}
          ...
          <Field label="Email Address" icon={<User className="w-3.5 h-3.5" />} error={errors.email}>
            <Input
              value={email}
...
function SecurityTab() {
  ...
  const [resetAlert,    setResetAlert]    = useState<{ type: 'success' | 'error'; msg: string } | null>(null)
  ...
  const handleResetRequest = async () => {
    if (!resetEmail.trim()) {
      setResetAlert({ type: 'error', msg: 'Please enter your email address' })
      return
    }
    ...
    try {
      ...
      setResetSent(true)
      setResetAlert({ type: 'success', msg: `Reset link sent to ${resetEmail}` })
    } catch (err: any) {
      setResetAlert({ type: 'error', msg: err?.response?.data?.message ?? 'Failed to send reset email' })
    } finally {
  ...
  return (
    <div className="space-y-5">
      ...
      <SectionCard
        ...
        description="Update your password while staying logged in"
        icon={<Lock className="w-4 h-4 text-blue-400" />}
      >
        <div className="space-y-5">
          {pwAlert && (
            <Alert type={pwAlert.type} message={pwAlert.msg} onDismiss={() => setPwAlert(null)} />
          )}
      ...
      <SectionCard
        ...
        description="Forgot your password? Send a reset link to your email"
        icon={<RefreshCw className="w-4 h-4 text-blue-400" />}
      >
        <div className="space-y-5">
          {resetAlert && (
            <Alert type={resetAlert.type} message={resetAlert.msg} onDismiss={() => setResetAlert(null)} />
          )}
            ...
            <div className="flex flex-col items-center text-center py-4 gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  We sent a reset link to <span className="text-foreground">{resetEmail}</span>.
                  It expires in 15 minutes.
                </p>
              </div>
              <button
                onClick={() => { setResetSent(false); setResetEmail(''); setResetAlert(null) }}
  ...
  return (
    <div className="space-y-5">
      ...
      >
        <div className="space-y-5">
          {createAlert && (
            <Alert type={createAlert.type} message={createAlert.msg} onDismiss={() => setCreateAlert(null)} />
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field
              ...
              error={nameError}
              hint="e.g. Laptop CLI, CI Pipeline, Home Server"
            >
              <Input
                value={name}
          ...
          <div className="flex items-start gap-2.5 p-3 rounded-lg bg-blue-500/5 border border-blue-500/15 text-xs text-muted-foreground">
            ...
            <span>
              Generated tokens are shown <strong className="text-foreground">once</strong> and
              cannot be retrieved. Store them securely in your{' '}
